...
public class LeaderApi {
    ...
    @RequestMapping(value = "/snow/select/tools/{userId}" , method = RequestMethod.GET)
    @ResponseBody
    public Map<String,Object> selectToolsTeam(@PathVariable(value = "userId") String teamId){
        logger.info("select/tools/{userId} : "+ JsonUtil.getJsonString(teamId));
        Tool tool =new Tool();
        tool.setTeamId(teamId);
        Map<String,Object> rtv = leaderService.selectToolsTeam(tool);
        return rtv;
    }

    @RequestMapping(value = "/snow/select/cars/{userId}" , method = RequestMethod.GET)
    @ResponseBody
    public Map<String,Object> selectCarsTeam(@PathVariable(value = "userId") String teamId){
        logger.info("select/cars/{userId} : "+ JsonUtil.getJsonString(teamId));

        Car car =new Car();
        car.setTeamId(teamId);

        Map<String,Object> rtv = leaderService.selectCarsTeam(car);
        return rtv;
    }


    @RequestMapping(value = "/snow/newTeam" , method = RequestMethod.POST)
    @ResponseBody
    public Map<String,Object> newTeam(@RequestBody Team team){
        logger.info("teamTask : "+ JsonUtil.getJsonString(team));

        Map<String,Object> rtv = leaderService.addTeam(team);
        return rtv;
    }
    ...
    @ResponseBody
    public Map<String,Object> snowNeeds(@RequestBody Needs needs){
        logger.info("applyContent : "+needs.getApplyContent());

        return leaderService.updateTeamApplyContent(needs);
    ...
    @ResponseBody
    public Map<String,Object> performScore (@RequestParam("userId") String userId,@RequestParam("score") double score){
   ...
   /* @RequestMapping(value = "/snow/teamTask" , method = RequestMethod.POST)
    @ResponseBody
    public Map<String,Object> teamTask (@RequestBody Team team){
        logger.info("POST performScore : "+ user.getUserId()+","+user.getUserTask());
